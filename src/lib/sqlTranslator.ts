 ...
 */
export function translateCreateTable(statement: string): { query: string; name: string; dependencies: string[]; booleanColumns: string[] } | null {
  ...
  try {
    ...
    if (ast.create_definitions) {
      for (const def of ast.create_definitions) {
        ...
        if (def.resource === 'column') {
          ...
          if (type === 'BOOLEAN') {
            const colName = (def.column as any)?.column || def.column;
            if (colName) {
              booleanColumns.push(String(colName).replace(/"/g, ''));
            }
          }

          def.definition.dataType = type;

          // Strip length, scale, parentheses, and expressions for types that do not support them in Postgres (like TEXT)
          const allowedConstraints = ['VARCHAR', 'CHAR', 'DECIMAL', 'NUMERIC', 'DOUBLE PRECISION', 'REAL'];
          if (!allowedConstraints.includes(type)) {
            delete (def.definition as any).length;
            delete (def.definition as any).scale;
            delete (def.definition as any).parentheses;
            delete (def.definition as any).expr;
            delete (def.definition as any).args;
          }
    ...
    let pgSql = parser.sqlify(ast, { database: 'postgresql' });
    
    // Clean up any type modifiers on TEXT (e.g. TEXT('flat', 'percentage') -> TEXT)
    pgSql = pgSql.replace(/\bTEXT\s*\([^)]*\)/ig, 'TEXT');
 ...
 */
function translateCreateTableRegex(statement: string): { query: string; name: string; dependencies: string[]; booleanColumns: string[] } | null {
  ...
  for (const line of lines) {
    ...
    if (/\bboolean\b/i.test(cleanLine)) {
      const colNameMatch = cleanLine.match(/^\s*"([a-zA-Z0-9_]+)"/);
      if (colNameMatch) {
        booleanColumns.push(colNameMatch[1]);
      }
    }
  ...
  return { query: pgQuery, name, dependencies, booleanColumns };
}

/**
 * Translates a MySQL INSERT INTO statement to PostgreSQL syntax.
 */
export function translateInsert(statement: string, booleanColumns: string[] = []): string {
  ...
  try {
    ...
    if (ast && ast.type === 'insert' && ast.columns && ast.values && booleanColumns.length > 0) {
      const colNames = ast.columns.map((c: any) => String(c).replace(/"/g, ''));
      
      // Find indexes of boolean columns
      const boolIndexes: number[] = [];
      colNames.forEach((colName: string, idx: number) => {
        if (booleanColumns.includes(colName)) {
          boolIndexes.push(idx);
        }
      });
      
      if (boolIndexes.length > 0) {
        // Map 1/0 values to boolean nodes
        const rows = (ast.values as any).values || ast.values;
        if (Array.isArray(rows)) {
          for (const row of rows) {
            if (row && Array.isArray(row.value)) {
              for (const idx of boolIndexes) {
                const valNode = row.value[idx];
                if (valNode) {
                  if (valNode.type === 'number' || valNode.type === 'origin') {
                    const val = Number(valNode.value);
                    if (val === 1) {
                      row.value[idx] = { type: 'bool', value: true };
                    } else if (val === 0) {
                      row.value[idx] = { type: 'bool', value: false };
                    }
                  } else if (valNode.type === 'single_quote_string') {
      ...
      // SQLify back to PostgreSQL
      pgQuery = parser.sqlify(ast, { database: 'postgresql' });
    }
  } catch (err) {
    console.warn(`AST insert translation failed: ${err}`);
  }
...
export function translateSqlFile(sqlContent: string): TranslationResult {
  ...
  // Pass 1: Parse all CREATE TABLE statements to discover table schema metadata
    ...
    if (isCreate) {
      ...
      if (translated) {
        if (tablesMap[translated.name]) {
          ...
          tablesMap[translated.name].booleanColumns = translated.booleanColumns;
        } else {
          tablesMap[translated.name] = {
            ...
            booleanColumns: translated.booleanColumns
          };
        }
      } else {
  ...
  // Pass 2: Parse INSERT INTO statements (using discovered boolean columns) and other statements
  for (const raw of rawStatements) {
    const cleanRaw = raw.trim();
    if (!cleanRaw) continue;
    ...
    if (isCreate) {
      ...
      if (tableName) {
        const boolCols = tablesMap[tableName]?.booleanColumns || [];
        const translated = translateInsert(cleanRaw, boolCols);
